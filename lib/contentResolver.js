var fs    = require('fs'),
    ...
    maestroUtils = require('maestro-lib/lib/utils'),
...
function readContentSync(meta) {
  ...
  if(meta.assembled) {
    try {
      ...
      var hasTranslations = fs.existsSync(meta.localePath);
      console.log('trans', meta.localePath, hasTranslations);

      if(hasTranslations) {
        translations = maestroUtils.getUnmergedLangsSync(meta.localePath);
        contents = appendContents(contents, "var langs = " + JSON.stringify(translations) + ";", meta.name + ".json");
        // XXX: this doesn't support having controls' locales in subdirectories. Do we care?
        var langscript = ["langs = FS.maestroControlsTranslations ? FS.maestroControlsTranslations['" + meta.appName + "']['" + meta.name + ".json'] || langs;",
                          "var locale = FS.locale || window.locale || 'en';",
                          "locale = typeof(locale) == 'string' ? locale : locale[0].split('-')[0];",
                          // XXX do we need this l1 variable? It gets put in the module's global namespace
                          "var l1 = langs[locale] || langs['en'];",
                          "var lang = $.extend({}, langs['en'], l1);",
                          "var polyglot = new Polyglot({locale: locale, phrases: lang});",
                          "var i18n = polyglot.t.bind(polyglot);"].join('\n');
        
        
        contents = appendContents(contents, langscript, "Injected code");
      }
...
function getContentMeta(pathPart, name, ext, type) {
  ...
  for(var i=0; i<finalPaths.length; ++i){
    ...
    if(fs.existsSync(fullPath)) {
      ...
      meta.localePath = path.resolve(path.join(meta.basePath, '../locales/controls', name + '.json'));
      meta.appName = path.basename(path.resolve(path.join(meta.basePath, '..')));
      break;
    }
  }
